...
from ai_solution_integration_test import load_data, preprocess_data, create_model, train_and_evaluate_model, partition_dataset
import mlflow
...
def test_load_data():
    ...
    # Check that each batch has images
    for images, labels in dataset.take(1):
        assert images.shape[0] > 0, "Batch has no images"
        assert labels.shape[0] > 0, "Batch has no labels"
...
def test_preprocess_data():
    ...
    assert len(list(train_ds)) > 0, "Training dataset is empty"
    assert len(list(val_ds)) > 0, "Validation dataset is empty"
    assert len(list(test_ds)) > 0, "Test dataset is empty"
...
def test_create_model():
    ...
    assert "conv2d" in layer_names, "Conv2D layer missing in model"
    assert "dense" in layer_names, "Dense layer missing in model"
    assert "softmax" in layer_names, "Softmax activation missing in the final layer"
